...
print('''=================
本程序通过深度学习计算等待期的属性：0-没有提及等待期，1-等待期内，2-等待期外，3-同时提及等待期内和外
输入参数一：训练材料（请及时更新训练材料train/dengdaiqi_test_xxxx.xlsx）
输入参数二：待预测材料（step1.2的输出benefit_output_splitted.txt）
输入参数三：词向量二进制文件
输入参数四：输出文件名（默认填写dengdaiqi_output.txt）
''')
...
saved_model_name = './DeepLearning_model_h5_file/dengdaiqi_model_%s.h5'%str_now[:15]
