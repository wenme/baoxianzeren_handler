...
print '本程序用于提取等待期的信息值，如果包含中文数字会转换成阿拉伯数字'
print '输入参数一：包含等待期标签的句子的excel文件或TXT文件'
print '输出文件：dengdaiqi_output.txt'
    ...
    for i in range(0, len(excel_file)):
        ...
        if label != 0:
            for word in quantifier_words:
                match = re.search(ur'([零一二三四五六七八九十百千万两]+)(%s)'%word, sentence)
                if match:
                    str_tmp = match.group(0)
                    if str_tmp[0] == u'十' or str_tmp[0] == u'百' or str_tmp[0] == u'千' or str_tmp[0] == u'万':
                        continue
                    value = chs_digit_convertor(match.group(1))
    
            if value == '':
                for word in quantifier_words:
                    match = re.search(ur'([0-9十百千万]+)(%s)'%word, sentence)
                    if match:
                        str_tmp = match.group(0)
                        if str_tmp[0] == u'十' or str_tmp[0] == u'百' or str_tmp[0] == u'千' or str_tmp[0] == u'万':
                            continue
...
output_file = open('dengdaiqi_output.txt', 'w')
