...
print '本程序用于提取给付金额的信息值，如果包含中文数字会转换成阿拉伯数字'
print '输入参数一：包含给付金额标签的句子的excel文件或TXT文件'
print '输出文件：jifujine_output.txt'
    ...
    for i in range(0, len(excel_file)):
        ...
        if label != 0:
            ...
            for word in quantifier_words:
                match = re.search(ur'([0-9\.零一二三四五六七八九十百千万两]+)(%s)'%word, sentence)
                if match:
                    str_tmp = match.group(0)
                    if str_tmp[0] == u'百' or str_tmp[0] == u'千' or str_tmp[0] == u'万':
                        continue
                    if str_tmp.find(u'%') != -1:
                        value = float(chs_digit_convertor(match.group(1))) / 100.0
                    elif str_tmp.find(u'‰') != -1:
                        value = float(chs_digit_convertor(match.group(1))) / 1000.0
                    else:
                        value = chs_digit_convertor(match.group(1))
    
            if value == '':
                match = re.search(ur'([十百千万零一二三四五六七八九]+)分之([十百千万零点一二三四五六七八九]+)', sentence)
                if match:
                    value = chs_digit_convertor(match.group(0))
