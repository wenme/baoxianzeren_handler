...
print '本程序用于提取被保人年龄的信息值，如果包含中文数字会转换成阿拉伯数字'
print '输入参数一：包含被保人年龄标签的句子的excel文件或TXT文件'
print '输出文件：beibaorennianling_output.txt'
...
quantifier_words = [u'、', u'/', u'和', u'周岁', u'岁']
    ...
    for i in range(0, len(excel_file)):
        ...
        if label != 0:
            ...
            for word in quantifier_words:
                match = re.findall(ur'([零一二三四五六七八九十百千万两]+)(%s)'%word, sentence)
                if len(match) > 0:
                    for i in match:
                        str_tmp = i[0]
                        if str_tmp == u'百' or str_tmp == u'千' or str_tmp == u'万':
                            continue
                        if sentence.find(str_tmp) == 0:
                            continue
...
output_file = open('beibaorennianling_output.txt', 'w')
