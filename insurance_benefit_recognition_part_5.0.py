...
print('''=================
本程序通过深度学习计算保单年度的属性：0-没有提及保单年度，1-xx之前，2-xx之后，3-XX之间，4-X至X
输入参数一：训练材料（请及时更新训练材料train/baodanniandu_test_xxxx.xlsx）
输入参数二：待预测材料（step1.2的输出benefit_output_splitted.txt）
输入参数三：词向量二进制文件
输入参数四：输出文件名（默认填写baodanniandu_output.txt）
''')
...
saved_model_name = './DeepLearning_model_h5_file/baodanniandu_model_%s.h5'%str_now[:15]
